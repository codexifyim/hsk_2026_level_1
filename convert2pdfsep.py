...
OUTPUT_PDF = "chinese_vocab_book.pdf"
...
for chinese, pinyin, meaning in words:
    # --- PAGE 1: CHINESE & PINYIN ---
    pdf.add_page()
    
    # Calculate height for just Chinese and Pinyin
    chinese_h = get_multicell_height(pdf, chinese, CHINESE_FONT_SIZE, CHINESE_LINE_HEIGHT)
    pinyin_h = get_pinyin_height(pdf, pinyin)
    top_half_height = chinese_h + CHINESE_TO_PINYIN_SPACING + pinyin_h
    
    # Center vertically
    pdf.set_y((page_height - top_half_height) / 2)
    ...
    # Calculate height for meaning
    meaning_h = get_multicell_height(pdf, meaning, MEANING_FONT_SIZE, MEANING_LINE_HEIGHT)
    
    # Center vertically
    pdf.set_y((page_height - meaning_h) / 2)
